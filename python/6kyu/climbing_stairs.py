...
def climbing_stairs(cost):
    steps = [0] * len(cost)
    steps[0] = cost[0] # base case
    steps[1] = min(cost[0], 0) + cost[1]

    for idx in range(2, len(steps)):
        # each decision is if we minimize our cost by taking two steps now or 1 step now.
...
def main():
    test.assert_equals(climbing_stairs([10, 15, 20]), 15)
    test.assert_equals(climbing_stairs([1, 100, 1, 1, 1, 100, 1, 1, 100, 1]), 6)
    test.assert_equals(climbing_stairs([0, 2, 2, 1]), 2)
    test.assert_equals(climbing_stairs([0, 2, 3, 2]), 3)
    test.assert_equals(climbing_stairs([0, 0, 0, 0, 0, 0]), 0)
