...
def reg_sum_hits(n, s):
    ...
    def n_perms(k,s,b):
        '''returns number of permutations of k numbers no bigger than b adding up to s(um)'''
        ct = 0
        for j in product(list(range(1,b+1)), repeat=k):
            if sum(j) == s:
                ct += 1
        return ct

    out = []
    for j in range(n,n*s+1):
        out.append([j,n_perms(n,j,s)])

    return out

def main():
    # 3 tetraedral dice
    test.assert_equals(reg_sum_hits(3, 4), [[3, 1], [4, 3], [5, 6], [6, 10],
    [7, 12], [8, 12], [9, 10], [10, 6], [11, 3], [12, 1]])
    
    # 4 tetrahedral dice
    test.assert_equals(reg_sum_hits(4, 4), [[4, 1], [5, 4], [6, 10], 
    [7, 20], [8, 31], [9, 40], [10, 44], [11, 40], [12, 31], [13, 20],
    [14, 10], [15, 4], [16, 1]])
    
    # 3 cubic dice
    test.assert_equals(reg_sum_hits(3, 6), [[3, 1], [4, 3], [5, 6], 
    [6, 10], [7, 15], [8, 21], [9, 25], [10, 27], [11, 27], [12, 25],
    [13, 21], [14, 15], [15, 10], [16, 6], [17, 3], [18, 1]])
    
    # 4 cubic dice
    test.assert_equals(reg_sum_hits(4, 6), [[4, 1], [5, 4], [6, 10],
